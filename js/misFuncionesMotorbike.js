function autoInicioCategoria(){
    ...
    $.ajax({
        url:"http://129.151.119.199:80/api/Category/all",
        type:"GET",
        datatype:"JSON",
...
function traerInformacionMotorbike() {
    $.ajax({
        url:"http://129.151.119.199:80/api/Motorbike/all",
...
function cargarDatosMotorbike(id) {
    $.ajax({
        dataType: 'json',
        url:"http://129.151.119.199:80/api/Motorbike/"+id,
    ...
    if($("#name2").val().length == 0 || $("#brand").val().length == 0 || $("#year").val().length == 0 || $("#description2").val().length == 0){
       alert("Todos los campos son obligatorios")
            ...
            $.ajax({
                ...
                url:"http://129.151.119.199:80/api/Motorbike/save",
    ...
    $.ajax(
        {
            dataType: 'json',
            data: dataToSend,
            url:"http://129.151.119.199:80/api/Motorbike/"+idElemento,
...
function actualizar(idElemento) {
    ...
    if($("#name2").val().length == 0 || $("#brand").val().length == 0 || $("#year").val().length == 0 || $("#description2").val().length == 0){
        ...
        $.ajax({
            ...
            data: dataToSend,
            contentType: "application/JSON",
            url:"http://129.151.119.199:80/api/Motorbike/update",
